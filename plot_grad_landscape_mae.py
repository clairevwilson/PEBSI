"""
Plots the MAE-loss AD gradient landscape from collect_grad_landscape_mae.csv.

Usage:
    python plot_grad_landscape_mae.py [input.csv] [output.png]
Defaults to grad_landscape_mae.csv and grad_landscape_mae.png.
"""
import sys
import numpy as np
import matplotlib
...
CSV_IN  = sys.argv[1] if len(sys.argv) > 1 else 'grad_landscape_mae.csv'
PNG_OUT = sys.argv[2] if len(sys.argv) > 2 else 'grad_landscape_mae.png'

data  = np.genfromtxt(CSV_IN, delimiter=',', names=True, dtype=None, encoding='utf-8')
wf    = data['wf'].astype(float)
loss  = data['loss_mae'].astype(float)
grad  = data['grad_mae'].astype(float)
sweep = data['sweep']
...
ax_loss.set_ylabel('MAE loss (m w.e.)')
ax_loss.grid(True, lw=0.4, alpha=0.5)
ax_loss.set_title('K53 / Kahiltna — AD gradient landscape (MAE loss)\n2015 - 2020 period')
