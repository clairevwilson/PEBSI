...
def expand_forcings(forcings, point_attrs):
    ...
    return forcings._replace(
        tempC=forcings.tempC[idx],
        tempK=forcings.tempK[idx],
        tp=forcings.tp[idx],
        prec=forcings.prec[idx],
...
def adjust_temperature(forcings, point_attrs, params):
    ...
    temp = forcings.tempC + lapse_rate * (point_attrs.elevation - temp_elev)
    return forcings._replace(tempC=temp, tempK=temp + params.celsius_to_kelvin)

def adjust_precipitation(forcings, point_attrs, params):
    """Applies elevation gradient and kp factor to precipitation."""
    tp = forcings.tp * (1 + params.precgrad * (point_attrs.elevation - point_attrs.median_elev)) * params.kp
    return forcings._replace(tp=tp, prec=tp / params.seconds_per_hour)
...
def adjust_longwave(forcings, point_attrs, params):
    ...
    lapse_rate = params.lapse_rate / 1000
    CTOK = params.celsius_to_kelvin
    LWin_elev = point_attrs.LWin_elev[point_attrs.cell_idx]
    temp_cell = forcings.tempC + lapse_rate * (LWin_elev - point_attrs.elevation)
    ...
    eps_site = emissivity(forcings.tempC, forcings.rh)
    eps_cell = emissivity(temp_cell, forcings.rh)
    delta = (eps_site * params.sigma_SB * (forcings.tempC + CTOK)**4
